...
public class BOLPageFunctional {
	...
	/**
	 * @author
	 * @return flag This method is used to click on create new BOL button
	 * @throws InterruptedException
	 */
	...
	/**
	 * @author
	 * @return flag This method is used to create new BOL
	 * @throws InterruptedException
	 */
...
//		{
		...
		if (flag) {
			...
			if (flag) {
				...
				webDB.getDriver()
						.findElement(By.xpath(BOLPageLocators.FROM_LOC_DRP_OPTIONS_LIST + "[" + oneoption + "]"))
						.click();
				...
				if (flag) {
					...
					if (oneoption == toloc_oneoption) {
						...
						webDB.getDriver()
								.findElement(By
										.xpath(BOLPageLocators.TO_LOC_DRP_OPTIONS_LIST + "[" + finalto_location + "]"))
								.click();
						Thread.sleep(2000);
						...
						webDB.getDriver()
								.findElement(
										By.xpath(BOLPageLocators.TO_LOC_DRP_OPTIONS_LIST + "[" + toloc_oneoption + "]"))
								.click();
						Thread.sleep(2000);
						log.logging("info", "Selected To location");
						flag = true;
						...
						if (flag) {
							...
							webDB.getDriver()
									.findElement(By.xpath(
											BOLPageLocators.SHIPPER_DRP_OPTIONS_LIST + "[" + shipper_oneoption + "]"))
									.click();
							Thread.sleep(2000);
...
//	}
	...
	/**
	 * @author
	 * @return flag This method is used to add an order to the BOL
	 * @throws InterruptedException
	 */
...
//			flag = webDB.sendTextToAnElement(BOLPageLocators.WIP_INPUTFIELD_BOLCOMPLETEPAGE, "5", ElementType.Xpath);
	...
	/**
	 * @author
	 * @return flag This method is used to get BOL Number from BOL details page
	 * @throws InterruptedException
	 */
	public String getBOLNumberFromBOLDetailsPage() {
		String BOLNumbertext = webDB.getTextFromElement(BOLPageLocators.BOL_NUMBER_FROM_BOLDETAILSPAGE,
				ElementType.Xpath);
	...
	public String getBOLNumberFromOrderDetailsPage() {
		String BOLNumbertext = webDB.getTextFromElement(BOLPageLocators.BOL_NUMBER_FROM_ORDERDETAILSPAGE,
				ElementType.Xpath);
		Pattern pattern = Pattern.compile("\\d+");
		Matcher matcher = pattern.matcher(BOLNumbertext);
		if (matcher.find()) {
	...
	/**
	 * @author
	 * @return flag This method is used to click on Add Order to BOL and Create BOL
	 *         button from Order details page
	 * @throws InterruptedException
	 */
		...
		if (flag) {
			...
			if (flag) {
				...
				if (flag) {
					...
					webDB.getDriver()
							.findElement(
									By.xpath(BOLPageLocators.SHIPPER_DRP_OPTIONS_LIST + "[" + shipper_oneoption + "]"))
							.click();
					Thread.sleep(2000);
					log.logging("info", "Selected Shipper");
					flag = webDB.isElementDisplayed(BOLPageLocators.SUBMIT_BOL_BUTTON, ElementType.Id);
	...
	/**
	 * @author
	 * @return flag This method is used to click on Add Order to BOL button from
	 *         Order details page
	 * @throws InterruptedException
	 */
	 ...
	 * @author
	 * @return flag This method is used to click on Add Order to BOL and Create BOL
	 *         button from Order details page and fill the carrier details
	 * @throws InterruptedException
	 */
	...
	public boolean verify_InputBOLFilter() throws InterruptedException {
		commonfunction.applyTextFilter(BOLPageLocators.STRING_BOL_COLDATA, BOLPageLocators.STRING_BOL_RESULTDATA,
				ElementType.Xpath, BOLPageLocators.STRING_BOL_INPUTFIELD_FILTER, ElementType.Xpath);
		return flag;
	}
