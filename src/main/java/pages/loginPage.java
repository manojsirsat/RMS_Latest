		...
		if (flag) {
			...
			if (flag) {
				...
				if (flag) {
					...
					if (flag) {
						flag = webDB.clickAnElement(CommonFunctionsLocators.SIGNIN_BUTTON, ElementType.Xpath);
						...
						Thread.sleep(2000);
						log.logging("info", "The credentials are incorrect");
						flag = false;
					}
				}
			}
		} else {
			log.logging("info", "Login page is not displayed");
			flag = false;
		}
